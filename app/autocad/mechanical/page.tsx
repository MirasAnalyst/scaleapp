      ...
      <div className="container mx-auto px-4 max-w-5xl py-12">
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-800 p-8 mb-10">
          ...
          <div className="space-y-6">
            ...
            {false && (
              <div className="border border-green-200 dark:border-green-900 rounded-xl p-5 bg-green-50 dark:bg-green-900/10 space-y-4">
                {generationState.specSummary && (
                  <div>
                    <p className="text-sm font-semibold text-green-700 dark:text-green-300 mb-3">
                      Mechanical Spec Snapshot
                    </p>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm text-green-700 dark:text-green-300">
                      <div>
                        <span className="block text-xs uppercase text-green-500 dark:text-green-400 tracking-wide">
                          Project
                        </span>
                        <span className="font-medium">{generationState.specSummary.project}</span>
                      </div>
                      <div>
                        <span className="block text-xs uppercase text-green-500 dark:text-green-400 tracking-wide">
                          Operating Context
                        </span>
                        <span>{generationState.specSummary.missionProfile}</span>
                      </div>
                      <div>
                        <span className="block text-xs uppercase text-green-500 dark:text-green-400 tracking-wide">
                          Revision
                        </span>
                        <span>{generationState.specSummary.revision}</span>
                      </div>
                      <div>
                        <span className="block text-xs uppercase text-green-500 dark:text-green-400 tracking-wide">
                          Subassemblies
                        </span>
                        <span>{generationState.specSummary.stages}</span>
                      </div>
                      <div>
                        <span className="block text-xs uppercase text-green-500 dark:text-green-400 tracking-wide">
                          Components
                        </span>
                        <span>{generationState.specSummary.parts}</span>
                      </div>
                      <div>
                        <span className="block text-xs uppercase text-green-500 dark:text-green-400 tracking-wide">
                          Generated
                        ...
                        <span>
                          {new Date(generationState.specSummary.generatedAtIso).toLocaleString()}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                {generationState.analysisChecks.length > 0 && (
                  <div>
                    <p className="text-sm font-semibold text-green-700 dark:text-green-300 mb-2">
                      Automated Sanity Checks
                    </p>
                    <ul className="space-y-2 text-sm text-green-700 dark:text-green-300">
                      {generationState.analysisChecks.map((check) => (
                        <li
                          key={check.checkId}
                          className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 rounded-lg bg-white/70 dark:bg-black/20 p-3 border border-green-200/60 dark:border-green-900/40"
                        >
                          <div>
                            <span className="font-semibold">{check.type.replace(/_/g, ' ')}</span>
                            <span className="ml-2">{check.summary}</span>
                            ...
                            className={`px-2 py-0.5 rounded text-xs font-semibold uppercase ${
                              check.status === 'pass'
                                ...
                                : check.status === 'warning'
                                  ? 'bg-yellow-200 text-yellow-900 dark:bg-yellow-900/60 dark:text-yellow-200'
                ...
                {generationState.artifacts.length > 0 && (
                  <div>
                    <p className="text-sm font-semibold text-green-700 dark:text-green-300 mb-2">
                      Download Artifacts
                    </p>
                    <ul className="space-y-3">
                      {generationState.artifacts.map((artifact) => (
                        <li
                          key={artifact.key}
                          className="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-white/80 dark:bg-black/30 border border-green-200/60 dark:border-green-900/40 rounded-lg p-3"
                        >
                          <div>
                            ...
                            <p className="text-xs text-green-600 dark:text-green-400">
                              {artifact.type.toUpperCase()} • {artifact.contentType.toUpperCase()} •{' '}
                              {artifact.storage === 's3' ? 'S3' : 'Local storage'}
                            </p>
                          </div>
                          <a
                            href={artifact.downloadUrl}
                            download={artifact.filename}
                            className="inline-flex items-center justify-center mt-3 sm:mt-0 px-3 py-2 text-sm font-medium text-white bg-green-600 hover:bg-green-700 rounded-md transition-colors"
                          >
                ...
                {generationState.warnings.length > 0 && (
                  <div className="border border-yellow-200 dark:border-yellow-900/60 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-4">
                ...
                {generationState.logs.length > 0 && (
                  <details className="bg-white/70 dark:bg-black/30 border border-green-200/60 dark:border-green-900/40 rounded-lg p-4">
                    <summary className="text-sm font-semibold text-green-700 dark:text-green-300 cursor-pointer">
                      CAD Worker Logs
                    </summary>
                    <div className="mt-3 bg-gray-900/5 dark:bg-gray-800/60 rounded-lg p-3 max-h-48 overflow-auto text-xs font-mono text-green-800 dark:text-green-200 space-y-1">
                      {generationState.logs.map((line, index) => (
                        <p key={`log-${index}`}>{line}</p>
                      ))}
                    </div>
                  </details>
                )}
                ...
                <p className="text-xs text-green-600 dark:text-green-400">
                  Tip: open the DXF in AutoCAD, AutoCAD LT, or any DWG-compatible viewer to refine layers, add dimensions, or generate DWG packages. STEP files import into FreeCAD, SolidWorks, and Inventor.
                </p>
              </div>
            )}
          </div>
