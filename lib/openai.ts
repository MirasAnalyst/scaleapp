...
function getOpenAIClient() {
  ...
  if (!apiKey) {
    throw new Error(
      "OPENAI_API_KEY is not set. Supply a valid key in your environment before generating CAD jobs.",
    );
  }
...
export async function generateBuildingSpec(prompt: string): Promise<BuildingSpecType> {
  try {
    ...
    const completion = await openai.chat.completions.create({
      model: "gpt-4o-mini",
      messages: [
        { role: "system", content: buildingSystemPrompt },
...
export async function generateRocketSpec(prompt: string): Promise<RocketSpecType> {
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      temperature: 0.1,
      messages: [
        { role: "system", content: rocketSystemPrompt },
        { role: "user", content: prompt },
      ],
      tools: [
        {
          ...
          function: {
            ...
            parameters: rocketFunctionParameters,
          },
        },
      ],
      tool_choice: { type: "function", function: { name: "produce_rocket_spec" } },
    });

    const toolCall = completion.choices[0]?.message?.tool_calls?.[0];
    if (!toolCall || toolCall.type !== "function" || toolCall.function.name !== "produce_rocket_spec") {
      throw new Error("No valid rocket spec tool call returned from OpenAI");
    }
