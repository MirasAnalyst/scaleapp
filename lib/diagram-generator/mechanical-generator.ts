...
export class MechanicalDiagramGenerator {
  ...
  private extractSpecifications(prompt: string): any {
    ...
    // Extract pipe sizes
    const pipeMatch = prompt.match(/(\d+)\s*[-]?\s*(inch|in|mm|cm|"|'')/i);
    if (pipeMatch) {
      specs.pipeSize = `${pipeMatch[1]} ${pipeMatch[2].replace(/['"]/g, 'inch')}`;
    }

    // Extract pipe materials
    const materialMatch = prompt.match(/(carbon steel|stainless steel|steel|copper|plastic|pvc|hdp|aluminum)/i);
    if (materialMatch) {
      specs.pipeMaterial = materialMatch[1];
    }
  ...
  private createComponentSpec(type: string, position: { x: number; y: number }, specs: any, index: number): ComponentSpec {
    // Try to find a tag that matches this equipment type
    let tag = this.generateTag(type, index + 1);
    
    if (specs.tags && specs.tags.length > 0) {
      // Look for a tag that matches the equipment type
      const typePrefix = this.getTypePrefix(type);
      const matchingTag = specs.tags.find((t: string) => t.startsWith(typePrefix));
      if (matchingTag) {
        tag = matchingTag;
      } else if (specs.tags[index]) {
        // Fallback to index-based assignment
        tag = specs.tags[index];
      }
    }
    ...
    // Add specifications to properties based on component type
    if (specs.flowRate && (type === 'pump' || type === 'compressor')) {
      properties.flowRate = specs.flowRate;
    }
    if (specs.pressure && (type === 'pump' || type === 'compressor' || type === 'tank' || type === 'boiler')) {
      properties.pressure = specs.pressure;
    }
    if (specs.temperature && (type === 'heat_exchanger' || type === 'boiler' || type === 'tank')) {
      properties.temperature = specs.temperature;
    }
    if (specs.power && (type === 'pump' || type === 'compressor' || type === 'turbine' || type === 'motor')) {
      properties.power = specs.power;
    }
    if (specs.pipeSize && type === 'pipe') {
      properties.pipeSize = specs.pipeSize;
    }
    if (specs.pipeMaterial && type === 'pipe') {
      properties.pipeMaterial = specs.pipeMaterial;
    }
  ...
  private getTypePrefix(equipmentType: string): string {
    ...
    return prefixes[equipmentType] || 'E';
  }
      ...
      case 'pump':
        return `
          <g transform="translate(${x}, ${y})">
            ...
            ${properties.flowRate ? `<text x="0" y="55" text-anchor="middle" class="text-xs fill-gray-500">${properties.flowRate}</text>` : ''}
            ${properties.pressure ? `<text x="0" y="70" text-anchor="middle" class="text-xs fill-gray-500">${properties.pressure}</text>` : ''}
          </g>`;
      ...
      case 'pipe':
        return `
          <g transform="translate(${x}, ${y})">
            ...
            <text x="0" y="5" text-anchor="middle" class="text-xs font-semibold fill-white">${properties.tag || 'L-101'}</text>
            <text x="0" y="40" text-anchor="middle" class="text-xs fill-gray-600">PIPE</text>
            ${properties.pipeSize ? `<text x="0" y="55" text-anchor="middle" class="text-xs fill-gray-500">${properties.pipeSize}</text>` : ''}
            ${properties.pipeMaterial ? `<text x="0" y="70" text-anchor="middle" class="text-xs fill-gray-500">${properties.pipeMaterial}</text>` : ''}
          </g>`;
