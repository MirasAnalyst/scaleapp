...
export class DXFGenerator {
  ...
  private addTitleBlock(title: string, prompt: string): void {
    ...
    // Title
    this.dxf.drawText(20, 280, 5, 0, title);
    
    // Prompt/description
    this.dxf.drawText(20, 270, 3, 0, `Generated from: ${prompt}`);
    
    // Generation info
    this.dxf.drawText(20, 260, 2.5, 0, `Generated: ${new Date().toLocaleString()}`);
    this.dxf.drawText(20, 250, 2.5, 0, 'ScaleApp - AI Generated Drawing');
    
    // Scale and units
    this.dxf.drawText(20, 240, 2.5, 0, 'Scale: 1:100');
    this.dxf.drawText(20, 235, 2.5, 0, 'Units: mm');
  }
  ...
  private addBoiler(x: number, y: number, properties: any): void {
    ...
    this.dxf.drawText(x, y - 25, 3, 0, properties.tag || 'BOILER');
    this.dxf.drawText(x, y - 30, 2.5, 0, properties.pressure || '100 bar');
  }
  ...
  private addValve(x: number, y: number, properties: any): void {
    ...
    // Tag
    this.dxf.setActiveLayer('TEXT');
    this.dxf.drawText(x, y - 15, 2.5, 0, properties.type?.replace('_', ' ').toUpperCase() || 'VALVE');
  }
